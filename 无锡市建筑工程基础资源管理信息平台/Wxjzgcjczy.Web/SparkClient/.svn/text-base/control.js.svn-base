/*
* 基于jQuery validation plug-in 1.7的界面验证功能扩展，群耀软件开发，请放心根据需要修改。
*/
...
$(function() {
    ...
    enabled_input_controls.filter("[mytype='decimal'],[mytype='double']").keydown(function(event) {
        ...
        //上面这句屏蔽掉了小键盘，为放开小键盘，改成下面这句代码。
        return ((event.keyCode > 47) && (event.keyCode < 58)) || (event.keyCode == 190) || (event.keyCode >= 96) && (event.keyCode <= 105) || (event.keyCode == 110) || keyControl(event);
    });
    ...
    /*  
    下面这段程序中对submitHandler的定义会屏蔽掉asp:ImageButton的事件(原因见备忘录中的说明)，
    而且似乎对验证功能也没什么影响，故注释掉。
    $("form").validate({
