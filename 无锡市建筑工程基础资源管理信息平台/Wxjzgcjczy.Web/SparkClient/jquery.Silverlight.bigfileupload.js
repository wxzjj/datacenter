...
function onSLLoad(plugin, userContext, sender) {
    ...
    /*
     plugin.Content.BigFileUpload，实际就是访问了silerlight组件。对这种访问方法，如果用模态（modal）窗口打开一个包括silverlight组件的页面
     在浏览器中会报“未指明的错误”，估计就是因为个别机器上浏览器的某些安全控制，访问不到silverlight组件。
     这时可以通过将silverlight组件所在站点加到浏览器的可信站点里，就应该可以正常打开了。
     或者也可以换成下面这种写法，不过颇有些后遗症，效果不甚理想。
    */
    //    plugin.document.applets[0].UpFile_Begined = function(sender, beginedEventArgs) { uploadFile_Begined(sender, beginedEventArgs, plugin, userContext); };
    //    plugin.document.applets[0].UpFile_Progressing = function(sender, progressingEventArgs) { uploadFile_Progressing(sender, progressingEventArgs, plugin, userContext); };
    //    plugin.document.applets[0].UpFile_Cancelled = function(sender, eventArgs) { uploadFile_Cancelled(sender, eventArgs, plugin, userContext); };
    //    plugin.document.applets[0].UpFile_Completed = function(sender, completedEventArgs) { uploadFile_Completed(sender, completedEventArgs, plugin, userContext); };
    //    plugin.document.applets[0].UpFile_Deleted = function(sender, eventArgs) { uploadFile_Deleted(sender, eventArgs, plugin, userContext); };
}
