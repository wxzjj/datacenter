var IMGFOLDERPATH = '../../JqueryOpenWindow/QQshow/images/'; //图片路径配置
...
Dialog.prototype.show = function() {
    if (this.Width <= 1) {
        this.Width = this.Width * document.body.scrollWidth;
    }
    if (this.Height <= 1) {
    ...
    if (this.btnSave) {
        arr.push("           	<input id='_ButtonOK_" + this.ID + "'  type='button' value='" + this.btnText + "'>");
    }
    if (this.btnCancel) {
    ...
    if (!bgdiv) {
        ...
        $E.hide(bgdiv);
        //pw.$T("frameset")[0].innerHTML = "";
        //alert(pw.$T("noframes")[0]);
...
if (isIE) {
    ...
    document.addEventListener("keypress", Dialog.onKeyDown, false);
