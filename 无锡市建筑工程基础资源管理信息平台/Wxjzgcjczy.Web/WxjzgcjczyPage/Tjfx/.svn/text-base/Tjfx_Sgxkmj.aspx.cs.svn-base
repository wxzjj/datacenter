...
{
    public partial class Tjfx_Sgxkmj : BasePage
    {
        private TjfxBLL BLL;
        ...
        private void searchData()
        {
            ...
            if (!string.IsNullOrEmpty(BdateEnd.SelectedValue) && !string.IsNullOrEmpty(BdateStart.SelectedValue))
            {
                ...
                {
                    this.WindowAlert("提示：发证年度范围(结束年度必须大于开始年度)!");
                    return;
                }
            }

            DataTable dt = BLL.RetrieveTjfx_SgxkTj_Zmj(BdateStart.SelectedValue, BdateEnd.SelectedValue, ssdq).Result;

            Chart1.ChartAreas["ChartArea1"].Area3DStyle.Enable3D = false;
            Chart1.Titles[0].Text = "年度施工许可证面积统计（单位：平方米）";
            Chart1.Series["Default"].LegendText = "施工许可证面积（平方米）";
            ...
            {
                title += dt.Rows[i]["nf"].ToString2() + "总数为:" + dt.Rows[i]["cou"].ToDouble() + "平方米,";
            ...
            Chart1.Series["Default"].ToolTip = "#AXISLABEL的施工许可证面积为#VAL平方米";
            ...
            MakeParetoChart(Chart1, "Default", "Pareto", d_add, "施工许可证面积增幅(%)", Color.Orange);
