...
{
    public partial class Tjfx_XmZmj_Jg : BasePage
    {
        private TjfxBLL BLL;
        ...
        private void searchData()
        {
            ...
            if (!string.IsNullOrEmpty(EdateEnd.SelectedValue) && !string.IsNullOrEmpty(EdateStart.SelectedValue))
            {
                if (EdateEnd.SelectedValue.ToInt32() <= EdateStart.SelectedValue.ToInt32())
                {
                    this.WindowAlert("提示：竣工年度范围(结束年度必须大于开始年度)!");
                    return;
                }
            }

            DataTable dt = BLL.RetrieveTjfx_XmTj_Zmj_Jg(ddl_Xmfl.SelectedValue, EdateStart.SelectedValue, EdateEnd.SelectedValue, ssdq).Result;

            Chart1.ChartAreas["ChartArea1"].Area3DStyle.Enable3D = false;
            Chart1.Titles[0].Text = "年度项目竣工总面积统计（单位：平方米）";
            Chart1.Series["Default"].LegendText = "项目竣工总面积（平方米）";
            ...
            {
                title += dt.Rows[i]["nf"].ToString2() + "总面积为:" + dt.Rows[i]["cou"].ToDouble() + "平方米,";
            ...
            Chart1.Series["Default"].ToolTip = "#AXISLABEL的项目竣工总面积为#VAL平方米";
            ...
            MakeParetoChart(Chart1, "Default", "Pareto", d_add, "项目竣工总面积增幅(%)", Color.Orange);
