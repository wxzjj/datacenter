...
{
    public partial class Tjfx_HtbaHtje : BasePage
    {
        private TjfxBLL BLL;
        ...
        private void searchData()
        {
            ...
            if (!string.IsNullOrEmpty(BdateEnd.SelectedValue) && !string.IsNullOrEmpty(BdateStart.SelectedValue))
            {
                ...
                {
                    this.WindowAlert("提示：合同签订年度范围(结束年度必须大于开始年度)!");
                    return;
                }
            }

            DataTable dt = BLL.RetrieveTjfx_HtBaTj_Htje(BdateStart.SelectedValue, BdateEnd.SelectedValue, ssdq,tblb).Result;

            Chart1.ChartAreas["ChartArea1"].Area3DStyle.Enable3D = false;
            Chart1.Titles[0].Text = "年度合同备案合同金额统计（单位：万元）";
            Chart1.Series["Default"].LegendText = "合同备案合同金额（万元）";
            ...
            {
                title += dt.Rows[i]["nf"].ToString2() + "总数为:" + dt.Rows[i]["cou"].ToDouble() + "万元,";
            ...
            Chart1.Series["Default"].ToolTip = "#AXISLABEL的合同备案合同金额为#VAL万元";
            ...
            MakeParetoChart(Chart1, "Default", "Pareto", d_add, "合同备案合同金额增幅(%)", Color.Orange);

            Chart1.ChartAreas["ChartArea1"].AxisY2.Interval = 10000;
