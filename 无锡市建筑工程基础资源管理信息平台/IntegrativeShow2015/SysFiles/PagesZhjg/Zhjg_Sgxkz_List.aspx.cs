...
{
    public partial class Zhjg_Sgxkz_List : BasePageGuanli
    {
        #region 基本配置
        // 常量   
        ...
        private void powerGridViewDataBind(int pageIndex)
        {
            if (bIsBeFrom)
            {
                ihb.SearchInfo(InstanceHead + this.Gdv_SgxkzInfo.ID, arrParas, this, this.Gdv_SgxkzInfo.ID,pageIndex,20);
            }
            else
            {
                ihb.SearchInfo(InstanceHead + this.Gdv_SgxkzInfo.ID, null, this, this.Gdv_SgxkzInfo.ID,pageIndex,20);
            }
        }
