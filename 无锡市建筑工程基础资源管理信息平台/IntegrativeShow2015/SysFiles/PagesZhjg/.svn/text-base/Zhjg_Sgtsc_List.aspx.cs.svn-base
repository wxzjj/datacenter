...
{
    public partial class Zhjg_Sgtsc_List : BasePageGuanli
    {
        #region 基本配置
        // 常量   
        ...
        private void powerGridViewDataBind(int pageIndex)
        {
            if (bIsBeFrom)
            {
                ihb.SearchInfo(InstanceHead + this.Gdv_SgtscInfo.ID, arrParas, this, this.Gdv_SgtscInfo.ID,pageIndex,20);
            }
            else
            {
                ihb.SearchInfo(InstanceHead + this.Gdv_SgtscInfo.ID, null, this, this.Gdv_SgtscInfo.ID,pageIndex,20);
            }
        }
