...
{
    public partial class Zhjg_Jgba_List : BasePageGuanli
    {
        #region 基本配置
        // 常量   
       ...
       // protected GridView gv;
        ...
        private void powerGridViewDataBind(int pageIndex)
        {
            if (bIsBeFrom)
            {
                ihb.SearchInfo(InstanceHead + this.Gdv_JgbaInfo.ID, arrParas, this, this.Gdv_JgbaInfo.ID,pageIndex,20);
            }
            else
            {
                ihb.SearchInfo(InstanceHead + this.Gdv_JgbaInfo.ID,null, this, this.Gdv_JgbaInfo.ID,pageIndex,20);
            }
        }
