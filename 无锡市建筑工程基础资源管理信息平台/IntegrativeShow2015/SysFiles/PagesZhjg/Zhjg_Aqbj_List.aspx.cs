...
{
    public partial class Zhjg_Aqbj_List : BasePageGuanli
    {
        #region 基本配置
        // 常量   
        ...
        private void powerGridViewDataBind(int pageIndex)
        {
            if (bIsBeFrom)
            {
                ihb.SearchInfo(InstanceHead + this.Gdv_AqbjInfo.ID, arrParas, this, "Gdv_AqbjInfo", pageIndex, 20);
            }
            else
            {
                ihb.SearchInfo(InstanceHead + this.Gdv_AqbjInfo.ID, null,this, "Gdv_AqbjInfo", pageIndex, 20);
            }
        }
