//----------------------------------------------------------------------------
		...
		strFrame+='        </td><td width=60 align=center style="font-size:12px;font-weight:bold;cursor:default" Author=meizz ';
		...
		strFrame+='<td width=48 align=center style="font-size:12px;font-weight:bold;cursor:default" Author=meizz onmouseover="style.backgroundColor=\'#D7E1F0\'" ';
		...
		strFrame+='             Author=meizz align=center><input Author=meizz style="cursor:hand"  type=button class=button value=昨天 onclick="parent.meizzYesterday()" onfocus="this.blur()" title="当前上一天日期" style="font-size: 12px; height: 20px; cursor:hand"><input Author=meizz style="cursor:hand"  type=button class=button value=今天 onclick="parent.meizzToday()" ';
		strFrame+='             onfocus="this.blur()" title="当前日期" style="font-size: 12px; height: 20px; cursor:hand"><input Author=meizz style="cursor:hand"  type=button class=button value=明天 onclick="parent.meizzTomorrow()" onfocus="this.blur()" title="当前下一天日期" style="font-size: 12px; height: 20px; cursor:hand"></td><td ';
		...
		function meizzYesterday()  //昨天 Button
		{
		...
		function meizzTomorrow()  //明天 Button
		{
		    var newdate=new Date();
            var newtimems=newdate.getTime()+(24*60*60*1000);
            newdate.setTime(newtimems);
		...
		{ var da = eval("odatelayer.meizzDay"+i)     //书写新的一个月的日期星期排列
			...
			{ 
				...
				if(i<firstday)		//上个月的部分
				{
					...
					da.title=(mm==1?yy-1:yy) + "年" + (mm==1?12:mm-1) +"月" + meizzWDay[i] + "日";
		...
		//					da.borderColorDark="#E4E3F2";  // 	选择日期边框颜色
				...
				{
					da.innerHTML="<font style=' color: #B5C5D2;'>" + meizzWDay[i] + "</font>";
					da.title=(mm==12?1+parseInt(yy):yy) + "年" + (mm==12?1:mm+1) +"月" + meizzWDay[i] + "日";
					da.onclick=Function("meizzDayClick(this.innerText,1)");
					if(!outDate)
				...
				else		//本月的部分
				{
					...
					da.title=yy + "年" + mm +"月" + meizzWDay[i] + "日";
