$(function() {
    $("input[autosearch='xmmc']").autocomplete({
        source: "/CFBPage/AutoSearch.aspx?s1=xmmc"
    });
    $("input[autosearch='jsdw']").autocomplete({
        source: "/CFBPage/AutoSearch.aspx?s1=jsdw"
    });
    $("input[autosearch='sgdw']").autocomplete({
        source: "/CFBPage/AutoSearch.aspx?s1=sgdw"
    });
});
...
function SysRandom(minValue, maxValue) {
    if (maxValue < minValue) return string.Empty;
