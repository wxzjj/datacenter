...
namespace SparkServiceDesk
{
     ...
     * 修改历史：
     ***************************************************/
    public class DataExchangeZhjgpt : System.Web.Services.WebService
    {
        ...
        {
            return ConfigurationManager.ConnectionStrings["ZHJGPTConnectionString"].ProviderName;
        }
        ...
        {
            return ConfigurationManager.ConnectionStrings["ZHJGPTConnectionString"].ConnectionString;
        }
        ...
        private string Update(DataRow dr, string tableName, string sbdqcode)
        {
            ...
            else
            {
                try
                {
                    ...
                    dt.Rows[0]["datastate"] = DataState.等待上传.ToInt32();
                    DB.Update(sql, sp, dt);
                }
                catch (Exception e)
                {
