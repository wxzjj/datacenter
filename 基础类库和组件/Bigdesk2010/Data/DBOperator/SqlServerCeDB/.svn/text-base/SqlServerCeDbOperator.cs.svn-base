...
namespace Bigdesk2010.Data
{
    ...
    /// SqlServerCe 数据库访问类	
    /// </summary>
    public class SqlServerCeDbOperator : DBOperator
    {
        #region 私有成员
        ...
        /// SQL 事务
        /// </summary>
        private SqlCeTransaction _dbTransaction;
        ...
        public SqlServerCeDbOperator(string connectionString, DataBaseType dataBaseType)
        {
            this._dbConnection = new SqlCeConnection(connectionString);
        ...
        private void AddDbParameter(SqlParameterCollection sqlParameterCollection, System.Data.SqlServerCe.SqlCeParameterCollection sqlServerCeSqlParameterCollection)
        {
            if (sqlParameterCollection == null || sqlServerCeSqlParameterCollection == null) return;

            for (int i = 0, j = sqlParameterCollection.Count; i < j; i++)
            {
                SqlCeParameter p = (SqlCeParameter)((ICloneable)sqlParameterCollection[i]).Clone();
                ...
                sqlServerCeSqlParameterCollection.Add(p);
            }
        }
        ...
        public override DataTable ExeSqlForDataTable(string selectSQL, SqlParameterCollection sqlParameterCollection, string dataTableName)
        {
            ...
            using (SqlCeDataAdapter ad = new SqlCeDataAdapter(selectSQL, this._dbConnection))
            {
        ...
        {
            int result = -1;
            SqlCeCommand cmd = new SqlCeCommand(commandSQL, this._dbConnection);
        ...
        public override bool Update(string selectSQL, SqlParameterCollection sqlParameterCollection, DataTable dataTable, int commandTimeout)
        {
            ...
            using (SqlCeDataAdapter ad = new SqlCeDataAdapter(selectSQL, this._dbConnection))
            {
                ...
                SqlCeCommandBuilder objComBuilder = new SqlCeCommandBuilder(ad);
                if (this._isTransacting)
                {
                    ad.SelectCommand.Transaction = this._dbTransaction;
        ...
        protected override string GetPagedQuerySQL(string querySQL, string orderby, PagedData pageData)
        {
            ...
            {
                case DataBaseType.SQLSERVER2008CE:
                    {
