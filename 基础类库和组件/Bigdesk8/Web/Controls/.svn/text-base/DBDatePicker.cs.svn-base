...
namespace Bigdesk8.Web.Controls
{
    ...
    /// 日期控件
    /// </summary>
    [ToolboxData("<{0}:DBDatePicker runat=server></{0}:DBDatePicker>")]
    public class DBDatePicker : TextBox, IDataItem
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        {
            writer.AddAttribute("mytype", "date");
            ...
            {
                //writer.AddAttribute("class", "required dateISO"); //这种写法输出的class会屏蔽掉控件中以CssClass引用的样式定义，也会屏蔽掉以Theme方式给出的样式定义，故换成下面的写法，缪卫华2012-11-23
                this.CssClass += " required dateISO";
            }
            else
            {
                //writer.AddAttribute("class", "dateISO");
                this.CssClass += " dateISO";
            }
