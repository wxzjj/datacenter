...
namespace Bigdesk8.Windows.Controls
{
    ...
    public static class UIUtility
    {
        ...
        //        {
        //            listControl..Add(new ListItem(obj.ToString(), obj.ToString()));
        //        }
        //        else
        //        {
        ...
        /// <param name="data">数据</param>
        /// <param name="isAddSpaceItem">是否要加入一个空项</param>
        public static void ComboBoxDataBind(this ComboBox comboBox, DataTable data, bool isAddSpaceItem)
        {
            ...
            foreach (DataRow dr in data.Rows)
            {
                ...
                {
                    case 1:
                        comboBox.Items.Add(new ListItem(dr[0].ToString(), dr[0].ToString()));
                        break;
                    case 2:
                        comboBox.Items.Add(new ListItem(dr[0].ToString(), dr[1].ToString()));
                        break;
                    default:
                        throw new Exception("ComboBoxDataBind 参数 data 不正确! data 中的 DataRow 至少要有一个字段.");
                }
            }
            if (isAddSpaceItem)
            {
        ...
        public static void SetControlReadOnly(this Control control, bool isReadOnly)
        {
            ...
            if (control is TextBox)
                (control as TextBox).ReadOnly = isReadOnly;
            else if (control is CheckBox)
                (control as CheckBox).Enabled = !isReadOnly;
            else if (control is ComboBox)
                (control as ComboBox).Enabled = !isReadOnly;
            else if (control is DateTimePicker)
                (control as DateTimePicker).Enabled = !isReadOnly;
